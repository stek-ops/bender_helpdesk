...
import { ListFilter, User, Calendar } from "lucide-react"
import { useTranslation } from "../hooks/useTranslation"

const statusColors: Record<string, string> = {
  new: "bg-[var(--b24-warning)]", in_progress: "bg-[var(--b24-info)]", resolved: "bg-[var(--b24-success)]", closed: "bg-[var(--b24-text-tertiary)]", cancelled: "bg-[var(--b24-danger)]",
}
...
export default function AdminTickets() {
  ...
  useEffect(() => {
    ...
    api.get("/tickets", { params }).then((res: any) => setTickets(res.data?.data || []))
  }, [filter])
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-[var(--b24-text)]">{t("admin_tickets_all")}</h1>
      ...
      <div className="bg-[var(--b24-card)] rounded-lg border border-[var(--b24-border)] shadow-sm overflow-hidden">
        <div className="flex items-center px-0 py-0 bg-[var(--b24-bg-light)] border-b border-[var(--b24-border)]">
          ...
          <div className="flex-1 flex items-center gap-4 px-4 py-2.5 text-xs font-medium text-[var(--b24-text-secondary)] uppercase tracking-wider">
            ...
            <span className="flex-1 min-w-0">{t("admin_tickets_header_name")}</span>
            <span className="w-[90px] shrink-0">{t("admin_tickets_header_author")}</span>
            <span className="w-[100px] shrink-0 text-center">{t("admin_tickets_header_status")}</span>
            <span className="w-[130px] shrink-0">{t("admin_tickets_header_executor")}</span>
            <span className="w-[80px] shrink-0 text-center">{t("admin_tickets_header_deadline")}</span>
            <span className="w-[90px] shrink-0 text-right">{t("admin_tickets_header_date")}</span>
          </div>
        </div>
        ...
        {tickets.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-[var(--b24-text-secondary)] text-sm">{t("admin_tickets_no_tickets")}</p>
          </div>
        ) : (
          tickets.map((ticket: any) => (
            <Link key={ticket.id} to={"/tickets/" + ticket.id}
              ...
              <div className={"w-[4px] self-stretch shrink-0 " + (statusColors[ticket.status] || "bg-[var(--b24-text-tertiary)]")} />
              <div className="flex-1 flex items-center gap-4 px-4 py-3 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-sm font-medium text-[var(--b24-text)] group-hover:text-[var(--b24-primary)] transition-colors truncate block">{ticket.title}</span>
                </div>
                <span className="w-[90px] shrink-0 text-sm text-[var(--b24-text-sidebar)] truncate">{ticket.user?.name}</span>
                <span className="w-[100px] shrink-0 text-center"><Badge>{t("status_" + ticket.status)}</Badge></span>
                ...
                <span className="w-[80px] shrink-0 text-center">
                  {ticket.due_date ? (
                    <span className={"text-xs " + (new Date(ticket.due_date) < new Date() && ticket.status !== "closed" && ticket.status !== "cancelled"
                      ? "text-[var(--b24-danger)] font-medium"
